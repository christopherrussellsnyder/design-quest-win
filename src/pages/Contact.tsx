        ...
        <nav className="border-b border-[hsl(var(--border))] px-4 py-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-6 text-sm text-[hsl(var(--muted-foreground))]">
              ...
              <Link to="/contact" className="text-[hsl(var(--primary))]">Contact</Link>
            </div>
          ...
          <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <a href="mailto:support@korexintelligencesystems.com" className="text-[hsl(var(--muted-foreground))] hover:text-white transition-colors">
              ...
              <div className="space-y-4">
                {contactReasons.map((reason, i) => (
                  <div key={i} className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl p-5 flex items-start gap-4">
                    <div className="w-10 h-10 rounded-lg bg-[hsl(var(--primary))]/10 flex items-center justify-center shrink-0">
                      <reason.icon className="w-5 h-5 text-[hsl(var(--primary))]" />
                    </div>
                    <div>
            ...
            <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl p-8">
              {submitted ? (
                <div className="flex flex-col items-center justify-center h-full text-center py-12">
                  ...
                  <p className="text-[hsl(var(--muted-foreground))] text-sm">We'll be in touch within 24 hours.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    <label className="block text-sm font-medium text-[hsl(var(--muted-foreground))] mb-2">Full Name *</label>
                    <input
                      type="text" value={name} onChange={(e) => setName(e.target.value)} required maxLength={100}
                      className="w-full bg-[hsl(var(--muted))] border border-[hsl(var(--border))] rounded-lg px-4 py-3 text-white placeholder-[hsl(var(--text-tertiary))] focus:outline-none focus:border-[hsl(var(--primary))] transition-colors"
                      placeholder="Your name"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[hsl(var(--muted-foreground))] mb-2">Email Address *</label>
                    <input
                  ...
                  <div>
                    <label className="block text-sm font-medium text-[hsl(var(--muted-foreground))] mb-2">Subject</label>
                    <select
                      value={subject} onChange={(e) => setSubject(e.target.value)}
                      className="w-full bg-[hsl(var(--muted))] border border-[hsl(var(--border))] rounded-lg px-4 py-3 text-white focus:outline-none focus:border-[hsl(var(--primary))] transition-colors"
                    >
                      {allowedSubjects.map(s => <option key={s} value={s}>{s}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-[hsl(var(--muted-foreground))] mb-2">Message *</label>
                    <textarea
                      value={message} onChange={(e) => setMessage(e.target.value)} required rows={4} maxLength={5000}
                      className="w-full bg-[hsl(var(--muted))] border border-[hsl(var(--border))] rounded-lg px-4 py-3 text-white placeholder-[hsl(var(--text-tertiary))] focus:outline-none focus:border-[hsl(var(--primary))] transition-colors resize-none"
