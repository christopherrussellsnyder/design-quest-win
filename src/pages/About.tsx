      ...
      <div className="min-h-screen bg-[hsl(var(--background))] text-[hsl(var(--foreground))]">
        ...
        <nav className="border-b border-[hsl(var(--border))] px-4 py-4">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <div className="hidden md:flex items-center gap-6 text-sm text-[hsl(var(--muted-foreground))]">
              ...
              <Link to="/about" className="text-[hsl(var(--primary))]">About</Link>
        ...
        <section className="py-24 px-4 text-center">
          <div className="max-w-4xl mx-auto">
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[hsl(var(--primary))] to-[hsl(var(--primary-light))]">Play to Win</span>
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}
              className="text-lg text-[hsl(var(--muted-foreground))] max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {beliefs.map((belief, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.15 }}
                  className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl p-8 hover:border-[hsl(var(--primary))]/50 transition-all">
                  <div className="w-12 h-12 rounded-xl bg-[hsl(var(--primary))]/10 flex items-center justify-center mb-5">
                    <belief.icon className="w-6 h-6 text-[hsl(var(--primary))]" />
                  </div>
                  <h3 className="text-lg font-bold mb-3">{belief.title}</h3>
                  <p className="text-[hsl(var(--muted-foreground))] text-sm leading-relaxed">{belief.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="py-24 px-4 bg-[hsl(var(--muted))]">
          <div className="max-w-5xl mx-auto">
            ...
            <p className="text-[hsl(var(--muted-foreground))] text-lg leading-relaxed max-w-3xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
              {capabilities.map((c, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}
                  className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-2xl p-6 hover:border-[hsl(var(--primary))]/50 transition-all">
                  <div className="flex items-center gap-3 mb-3">
                    <c.icon className="w-5 h-5 text-[hsl(var(--primary))]" />
                    <h3 className="font-bold">{c.title}</h3>
                  </div>
                  <p className="text-[hsl(var(--muted-foreground))] text-sm leading-relaxed">{c.description}</p>
                </motion.div>
              ))}
            </div>
